...
class Calculate:
    def __init__(self, err_lab, table):
    ...
    def moment_load_func(self, c, vect,
                         moment_of_force):
        sA = 0
        out = []
        for i in range(len(self.bolt['name'])):
...
class OutCalc:
    ...
    """
    def __init__(self, calc, inpt, table):
        self.table = table
        self.calc = calc
        self.inpt = inpt
    ...
    def create_dataframe(self):
        ...
        tab_data['RF [-]'] = self.calculate_rf(tab_data['tau [MPa]'])
        tab_data['sigma1 [MPa]'] = self.calculate_sigma(
            tab_data['F [N]'], self.table.bolt_info['t1[mm]'])
        tab_data['RF1 [-]'] = self.calculate_rfi(
            tab_data['sigma1 [MPa]'], float(self.inpt.object1['Fbry[MPa]'].get()))
        tab_data['s2 [MPa]'] = self.calculate_sigma(
            tab_data['F [N]'], self.table.bolt_info['t2[mm]'])
        tab_data['RF2 [-]'] = self.calculate_rfi(
            tab_data['s2 [MPa]'], float(self.inpt.object2['Fbry[MPa]'].get()))
