...
class InputTable:
    ...
    def construct_table(self, table_type):
        ...
        err_lab.grid_remove()
        d = {}
        d['nrows'] = 1 if not info[samplekey] else len(info[samplekey])
        ...
        def add_row():
            ...
            for column in range(len(entrys)):
                entrys_id[d['nrows']][column] = tkinter.Entry(
                    inner_frame,
                    width=self.choose_width(column, entry_width),
                    borderwidth=2,
                    relief="groove",
                    justify="center")
                entrys_id[d['nrows']][column].grid(row=d['nrows'] + 1,
                                                 column=column,
                                                 padx=0,
                                                 pady=0)
            d['nrows'] += 1
        ...
        def remove_row():
            ...
            if d['nrows'] > 1:
                for column in range(len(entrys)):
                    entrys_id[d['nrows'] - 1][column].grid_remove()
                entrys_id.pop()
                d['nrows'] -= 1
        ...
        for row in range(d['nrows']):
            for column in range(len(entrys)):
                ...
                if info[samplekey] and row < d['nrows']:
